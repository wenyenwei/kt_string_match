...
import json
with open('res_edit_distance_default.txt', 'w') as file:
...
with open('evaluation.txt', 'a') as file:
    # precision
    # recall
     file.write(json.dumps({"precision_edit_distance_default": (float(true_count) / count_all)})) 
     file.write(json.dumps({"recall_edit_distance_default":(float(true_count) / len(res_dict))})) 
